...
export class UserService implements IUserServiceInterface {
  constructor(private userRepository: IUserRepositoryInterface, private productRepository:IProductsRepository) {}
  ...
  async updateUserRoleToAdmin(userId: string, newAdmin: UpdateUserDTO):Promise<User | null>{
    ...
    const admin = await this.userRepository.updateUserRoleToAdmin(userId, newAdmin)
    return admin 

  }
  async sendJewelryToUser(idUser: string, jewel: UpdateUserDTO ): Promise<User | null> {
  ...
  async addProducts(idUser: string, idProducts: string):Promise<User> {
    ...
    /* if(typeof userId.jewelsAmount !== 'number' || typeof productsId.value !== 'number' ) throw new Error("invalid data type") */
    const userJewelsAmount = userId.jewelsAmount as number

    if (userJewelsAmount < productsId.value) {
